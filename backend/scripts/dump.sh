...
# ダンプファイルのパスを設定
GEOJSON_FILE="safety_data/water.geojson"
...
if docker cp "$GEOJSON_FILE" "$CONTAINER_NAME:/safety_data/water.geojson"; then
    ...
    DO $do$
        ...
        BEGIN
            FOR feature IN 
                ...
                FROM (SELECT pg_read_file('/safety_data/water.geojson')::jsonb AS content) AS t
            LOOP
                properties := feature -> 'properties';
                geometry := feature -> 'geometry';
                
                INSERT INTO "Fukuoka_water_info"(prefecture, prefecture_code, flood_level, geometry)
                VALUES (
                    properties->>'A40_001',
                    properties->>'A40_002',
                    properties->>'A40_003',
